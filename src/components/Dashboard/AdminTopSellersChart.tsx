        ...
        {sellers.length === 0 ? (
          ...
          sellers.map((seller, index) => {
            ...
            return (
              <div 
                key={index} 
                className="group relative p-3 rounded-xl bg-gradient-to-r from-muted/40 to-muted/20 hover:from-muted/60 hover:to-muted/40 dark:from-muted/20 dark:to-muted/5 dark:hover:from-muted/30 dark:hover:to-muted/15 transition-all duration-300 animate-fade-in hover:shadow-md hover:scale-[1.01]"
                style={{ animationDelay: `${index * 100}ms` }}
              >
                <div className="flex items-center gap-3">
                  {/* Avatar with initials */}
                  <div className={`relative flex-shrink-0 w-12 h-12 rounded-full ${getAvatarColor(index)} flex items-center justify-center text-white font-bold text-sm`}>
                    {getInitials(seller.name)}
                    {/* Medal overlay for top 3 */}
                    {index < 3 && (
                      <div className={`absolute -top-1 -right-1 ${index === 0 ? 'animate-pulse' : ''}`}>
                        {getMedalIcon(index)}
                      </div>
                    )}
                  </div>
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between mb-1.5">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="font-semibold text-foreground truncate text-sm">{seller.name}</span>
                      </div>
                    ...
                    <div className="relative h-2.5 bg-muted/50 dark:bg-muted/30 rounded-full overflow-hidden">
                      <div 
                    ...
                    <div className="flex justify-between mt-1.5 text-xs">
                      ...
                      <span className="font-bold text-emerald-600 dark:text-emerald-400">{formatNumber(seller.revenue)} HTG</span>
                    </div>
                  </div>
                </div>
              </div>
